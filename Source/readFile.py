...
def readPlaylistDirectory(path, genre):
    ...
    for root, subDirectories, files in os.walk(path):
        for songFile in files:
            currentFeatures, currentName = readFile(path + "/" +  songFile)
            result.append(currentFeatures)
...
def readGenreDirectory(path):
    ...
    for root, subDirectories, files in os.walk(path):
        for subDirectory in subDirectories:
            playlistTuple = readPlaylistDirectory(path + "/" + subDirectory, currentGenre)
            result.append(playlistTuple[0])
            names.append(playlistTuple[1])
    return result, names
    ...
    for root, subDirectories, files in os.walk(os.path.abspath("../Dataset")):
        for subDirectory in subDirectories:
            if subDirectory == genres[tag]:
                result = readGenreDirectory("../Dataset/" + subDirectory)[0]
        break
